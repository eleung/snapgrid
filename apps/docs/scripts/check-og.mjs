...
// major.minor than the current @snapgridjs/react version. Keeps the social card's
...
if (stamped !== expected) {
  console.error(
    `✗ OG card is stale: rendered for v${stamped}, but @snapgridjs/react is ${version} (v${expected}).\n  Run \`pnpm assets\` and commit the refreshed og.png + og.version.`,
  );
  process.exit(1);
}
