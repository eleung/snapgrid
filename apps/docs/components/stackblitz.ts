...
//
// Live once @snapgrid/react is published to npm — the sandbox does a real
// `npm install @snapgrid/react`, so before publish it opens but can't resolve
// the package.
...
const DEP_VERSIONS: Record<string, string> = {
  react: "^18.3.1",
  "react-dom": "^18.3.1",
  "@snapgrid/react": "latest",
  "@snapgrid/extras": "latest",
  "@snapgrid/core": "latest",
  "@dnd-kit/react": "^0.4.0",
  "@dnd-kit/dom": "^0.4.0",
...
function packageJson(name: string, raw: string): string {
  const deps: Record<string, string> = {
    "@snapgrid/react": DEP_VERSIONS["@snapgrid/react"],
