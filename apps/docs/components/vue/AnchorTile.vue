...
// The lock ⟷ pin toggle tile: static (others flow around it); PINNED adds `isDraggable`
// so it can be dragged by the grip. The grip is ALWAYS the drag handle (setHandleRef) so
// dnd-kit puts its role + aria-disabled on the grip, not the cell — keeping the toggle a
// clean, clickable sibling. Icons are lucide paths inlined (no icon dep). Mirrors the
// React/Svelte `AnchorTile`.
    ...
    <div class="dg-tile dg-tile--static dg-anchor" :class="{ 'dg-anchor--pinned': pinned }">
      <div class="dg-anchor__head">
        ...
        <button
          ...
          @click="onTogglePin"
        >
          <svg v-if="pinned" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
            <path d="M12 17v5" />
