...
// The Vue examples gallery mounts native @snapgridjs/vue demos as client-side islands
// (vue-loader compiles the SFCs at build time; <VueDemo> mounts them after hydration).
// Like the React and Svelte galleries, the demos are headless (useGridContainer +
// useGridItem), so their surfaces/tiles carry the demo's `.dg-grid`/`.dg-cell` hooks —
// NOT the component layer's `.snapgrid` classes. These smoke-test that the islands
// mount and the grid engine works through the Vue binding.

test.beforeEach(async ({ page }) => {
  await page.goto("/vue/examples/");
  // The headless demos render `.dg-cell` tiles; wait for the islands to mount.
  await page.waitForSelector("main .dg-cell");
  await page.waitForTimeout(400);
});

test("every vue demo island mounts without a client error", async ({ page }) => {
  const errors: string[] = [];
