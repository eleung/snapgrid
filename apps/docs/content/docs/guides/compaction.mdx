...
```tsx
import { GridLayout, verticalCompactor, horizontalCompactor, noCompactor } from "@snapgrid/react";

<GridLayout compactor={horizontalCompactor} /* … */ />;
```
...
Exported from `@snapgrid/react` (re-exported from the core engine):
...
`@snapgrid/extras` adds variable-height packing styles. They repack **all** items by reading order,
so the dropped cell influences order while the algorithm decides final positions.

```tsx
import { masonryCompactor, gravityCompactor, shelfCompactor } from "@snapgrid/extras";
```
...
`@snapgrid/extras` also re-exports react-grid-layout's **O(n&nbsp;log&nbsp;n)** vertical and
...
```tsx
import { fastVerticalCompactor, fastHorizontalCompactor } from "@snapgrid/extras";
...
```tsx
import type { Compactor, Layout } from "@snapgrid/react";
