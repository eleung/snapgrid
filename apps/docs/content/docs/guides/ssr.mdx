...
<Callout type="info">
  The drag overlay is purely client-side: there's no active drag during SSR, so `<GridDragOverlay>`
  renders nothing on the server and the floating preview appears only once a drag begins in the
  browser. Nothing to guard.
</Callout>
