...
description: Migrating from react-grid-layout v1 (WidthProvider + flat props) to snapgrid — a prop-by-prop map, plus the three behaviours that differ. (v2 is nearly a no-op.)
---
...
<Callout type="info">
  **On react-grid-layout v2?** This guide isn't really for you. v2 already uses the composable
  `gridConfig` / `dragConfig` / `compactor` objects and `useContainerWidth` that snapgrid mirrors, so
  migrating is essentially **changing the import** (and renaming `ReactGridLayout` → `GridLayout`).
  See the side-by-side diff on the [home page](/). The rest of this guide targets **v1** — the
  `WidthProvider` HOC and flat-prop API — where there's actual mapping to do.
</Callout>
...
## A v1 drop-in is on the roadmap

The mapping above is by hand for now. A planned **`@snapgridjs/react/legacy`** entry will accept v1's
`WidthProvider` HOC and flat props directly, so you can change one import and migrate to the modern
API at your own pace — see the [roadmap](/roadmap).
