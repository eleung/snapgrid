...
`useGridResizeHandle(itemId, axis, group)` models a handle as its own draggable. Spread its `ref` and
...
| `isResizable` | Per-item on/off, overriding the grid default. `static` items never resize. |
