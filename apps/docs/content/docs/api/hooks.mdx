...
| `gridConfig` | `Partial<GridConfig>` | see [GridConfig](/docs/api/types#gridconfig) | Columns, row height, margins, padding, maxRows. Unset fields use defaults. |
| `dragConfig` | `DragConfig` | — | [Drag behaviour](/docs/guides/dragging): `handle` (CSS selector — headless prefers [`handleRef`](#usegriditem)), `cancel`, `threshold`, `bounded`, `snapToGrid`. |
| `resizeConfig` | `ResizeConfig` | `{ handles: ["se"] }` | [Resize behaviour](/docs/guides/resizing): which handle axes are allowed. |
| `dropConfig` | `DropConfig` | `{ enabled: false }` | [Accept external draggables](/docs/guides/external-drop): `enabled`, `defaultItem`, `accept`. |
| `compactor` | `Compactor` | `verticalCompactor` | [Packing algorithm](/docs/guides/compaction). Swap at runtime. |
...
| `onDragStart` `onDrag` `onDragStop` | `GridEventCallback` | — | [Drag lifecycle](/docs/guides/events) `(layout, oldItem, newItem, placeholder, event, node)`. |
...
## No useGridDragOverlay

Every hook above hands you values — a `ref`, a `style`, some state — to spread onto an element **you**
render. The floating drag preview doesn't fit that shape, so there's no `useGridDragOverlay` hook.
It's a **component**: [`<GridDragOverlay>`](/docs/api/components#griddragoverlay).

That mirrors dnd-kit. Its overlay is [`DragOverlay`](https://next.dndkit.com) — a component, not a
hook — because the overlay **owns and renders its own element**, portaling it to the top layer so the
dragged tile floats above everything and crosses between grids unclipped (while the in-grid tile
hides). There's no element of yours to attach a `ref` to, so there's nothing for a hook to return.
`<GridDragOverlay>` is snapgrid's thin wrapper over it: it resolves the dragged item for the
render-prop and bakes in the out-of-flow style so the overlay can't flash full-width on drop.

```tsx
import { GridDragOverlay } from "@snapgridjs/react";

// One overlay, rendered under your DragDropProvider as a sibling of the grid host.
<GridDragOverlay>
  {({ item }) => (item ? <div className="tile">{item.i}</div> : null)}
</GridDragOverlay>
```

`<GridLayout>` renders one for you; in the headless layer you render it yourself. See the
[`GridDragOverlay` reference](/docs/api/components#griddragoverlay) and
[Headless usage](/docs/guides/headless#dragoverlay).
