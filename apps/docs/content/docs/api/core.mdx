...
description: The framework-agnostic @snapgrid/core engine and the @snapgrid/extras packers.
---
...
The framework-agnostic layout engine. `@snapgrid/react` is a thin binding over it, so you rarely
...
<Callout type="info">
  `@snapgrid/core` is a thin adapter over `react-grid-layout/core` (re-exported) plus a small set of
