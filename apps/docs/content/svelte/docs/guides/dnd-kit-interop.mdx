...
description: Drag between a snapgrid grid and the wider dnd-kit ecosystem — a createSortable tray, a kanban list — under one provider, with snapMove.
---

import { Callout } from "nextra/components";
import { SortableGridDemo } from "@/components/demos-svelte";

# dnd-kit interop

A snapgrid grid **is** dnd-kit. Every tile is a real `createSortable`, so a grid composes with the
rest of the dnd-kit ecosystem under one `DragDropProvider`: drag a card from a `createSortable` tray
...
in-grid moves, [cross-grid](/svelte/docs/guides/cross-grid) hand-off, and [resizing](/svelte/docs/guides/resizing),
and reports results through [`onLayoutChange`](/svelte/docs/guides/events). That still applies here: an
...
<Callout type="warning">
  ...
  DOM node mid-drag; if you only update state on drop, Svelte's `{#each}` reconciler fights that
  reparent and throws `removeChild`. Reducing live keeps Svelte in sync with dnd-kit the whole drag.
</Callout>

## One provider

The grid and the foreign sortable must share a single dnd-kit manager, so they live under one
`DragDropProvider` (or snapgrid's [`SnapGridGroup`](/svelte/docs/guides/cross-grid), a thin wrapper). Import
the dnd-kit primitives from `@snapgridjs/svelte` so everything resolves the *same* dnd-kit instance —
a second copy of `@dnd-kit/svelte` is a separate provider context and its drags never reach the grid.
...
By default a grid only accepts grid tiles and [external-drop](/svelte/docs/guides/external-drop)
draggables. Opt a foreign sortable in with the `accept` option on `createGridContainer` — it widens
...
const container = createGridContainer(() => ({
  layout,
  width,
  onLayoutChange: (next) => (grid = next),
  // Accept the tray's cards (type "tray-card") as drop targets.
  accept: (source) => source.type === "tray-card",
}));
```

The tray card is a plain `createSortable`. Give it a `type` you'll branch on, and have it `accept` grid
...
<DragDropProvider
  onDragOver={(event) => {
    ...
    // Grid tile → tray: remove it AND re-pack the hole, then insert into the tray.
      ...
      grid = removeItemWithCompactor(grid, id, { compactor: verticalCompactor, cols: GRID.cols });
