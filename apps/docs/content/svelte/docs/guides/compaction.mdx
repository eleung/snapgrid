...
import { CompactorDemo } from "@/components/demos-svelte";

# Compaction & packing

After every move, resize, insert, or remove, the layout is re-packed by a **`Compactor`**. Pass one
as the `compactor` option to `createGridContainer` (or, identically, as a prop on
[`<GridLayout>`](/svelte/docs/guides/component-layer)); swap it any time. It's just a value.

```ts
import { createGridContainer, verticalCompactor, horizontalCompactor, noCompactor } from "@snapgridjs/svelte";

const container = createGridContainer(() => ({ compactor: horizontalCompactor, /* … */ }));
```
...
Exported from `@snapgridjs/svelte` (re-exported from the core engine):
...
const container = createGridContainer(() => ({ compactor: fastVerticalCompactor, /* … */ }));
```
...
```ts
import type { Compactor, Layout } from "@snapgridjs/svelte";
