...
`createGridContainer` (and, identically, props on [`<GridLayout>`](/svelte/docs/guides/component-layer)).
...
```ts
const container = createGridContainer(() => ({ onLayoutChange: (next) => (layout = next), /* … */ }));
```
...
`onLayoutChange`. See [External drop](/svelte/docs/guides/external-drop).
