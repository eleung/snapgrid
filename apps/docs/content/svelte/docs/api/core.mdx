...
The framework-agnostic layout math. The dnd-kit engine (`@snapgridjs/dnd`) and the Svelte binding
build on it, so you rarely import it directly. Reach for it when integrating snapgrid into a
non-Svelte renderer, building custom tooling, or testing layout logic in isolation.
...
The state machine the Svelte layer drives during an interaction:
...
<Callout type="warning">
  ...
  [Compaction](/svelte/docs/guides/compaction#extra-packers).
</Callout>
