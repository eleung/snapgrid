...
The framework-agnostic layout math. The dnd-kit engine (`@snapgridjs/dnd`) and the Vue binding
build on it, so you rarely import it directly. Reach for it when integrating snapgrid into a
non-Vue renderer, building custom tooling, or testing layout logic in isolation.
...
The state machine the Vue layer drives during an interaction:
...
<Callout type="warning">
  ...
  [Compaction](/vue/docs/guides/compaction#extra-packers).
</Callout>
