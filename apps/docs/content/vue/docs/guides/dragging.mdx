...
import { DragHandleDemo, SnapDemo } from "@/components/demos-vue";
...
prop on the [`<GridLayout>`](/vue/docs/guides/component-layer) shell):
...
| `handle` | `string` | — | CSS selector for a drag handle inside each item — only pointer-downs within it start a drag. Mainly for `<GridLayout>`; **headless tiles should prefer [`setHandleRef`](#drag-handles)** instead. |
...
**Turnkey — `dragConfig.handle`.** `<GridLayout>`'s item slot is plain markup with no function ref to
...
```js
const { setRef, style, group } = useGridContainer(() => ({ dragConfig: { handle: ".grip" }, /* … */ }));
```

<Callout type="warning">
  Use **one or the other** — `setHandleRef` *or* the selector. If you set both, a drag has to satisfy
...
- `static: true` anchors an item — others flow around it, and by default it can't be dragged or
  ...
  [Static items](/vue/docs/guides/static-items).
...
`(layout, oldItem, newItem, placeholder, event, node)`. See [Events & lifecycle](/vue/docs/guides/events).
