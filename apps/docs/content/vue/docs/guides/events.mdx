...
`useGridContainer` (and, identically, props on [`<GridLayout>`](/vue/docs/guides/component-layer)).
...
```js
const { setRef, style, group } = useGridContainer(() => ({ onLayoutChange: (next) => (layout.value = next), /* … */ }));
```
...
`onLayoutChange`. See [External drop](/vue/docs/guides/external-drop).
