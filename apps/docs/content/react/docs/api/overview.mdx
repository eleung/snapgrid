...
| Export | Returns |
| --- | --- |
| [`useGridContainer`](/react/docs/api/hooks#usegridcontainer) | `{ containerProps, group, isDropTarget, controller }` — the grid host |
| [`useGridItem`](/react/docs/api/hooks#usegriditem) | `{ ref, handleRef, style, isDragging, item }` |
| [`useGridResizeHandle`](/react/docs/api/hooks#usegridresizehandle) | `{ ref, handleProps, isResizing }` |
| [`useGridPlaceholder`](/react/docs/api/hooks#usegridplaceholder) | `{ item, style } \| null` |
| [`useContainerWidth`](/react/docs/api/hooks#usecontainerwidth) | `{ width, mounted, containerRef }` |
| [`useResponsiveLayout`](/react/docs/api/hooks#useresponsivelayout) | `{ breakpoint, cols, layout, onLayoutChange }` |

### Components (turnkey)

| Export | Purpose |
| --- | --- |
| [`GridLayout`](/react/docs/api/components#gridlayout) | Drop-in controlled grid with keyed children (bundles the provider). |
| [`ResponsiveGridLayout`](/react/docs/api/components#responsivegridlayout) | Per-breakpoint layouts. |
| [`GridItem`](/react/docs/api/components#griditem) | A positioned tile with default markup + handles. |
| [`GridPlaceholder`](/react/docs/api/components#gridplaceholder) | Default landing-cell marker. |
| [`SnapGridGroup`](/react/docs/api/components#snapgridgroup) | Shares one provider across sibling grids (cross-grid). |
...
[floats itself](/react/docs/guides/headless#floating) (dnd-kit lifts it into the top layer), so the common
...
`snapMove(layout, event)` — the reducer for [dnd-kit interop](/react/docs/guides/dnd-kit-interop):
...
`ResponsiveLayouts`, `PositionParams`, `ResizeHandleAxis`. See [Types & config](/react/docs/api/types).
...
[Core engine & extras](/react/docs/api/core).
...
[Core engine & extras](/react/docs/api/core#snapgridjsextras).
