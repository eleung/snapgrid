...
The component layer — a thin shell in the react-grid-layout style over the [hooks](/react/docs/api/hooks).
...
draggables, use the hooks. See the [Component layer guide](/react/docs/guides/component-layer) for usage;
this page is the prop reference.

A dragged tile [floats itself](/react/docs/guides/headless#floating), so there's no overlay component to
render at either layer. If you need a *separate* floating preview, dnd-kit's
[`DragOverlay`](https://next.dndkit.com) is re-exported from `@snapgridjs/react`.
...
| `gridConfig` | `Partial<GridConfig>` | see [GridConfig](/react/docs/api/types#gridconfig) | Columns, row height, margins, padding, maxRows. |
| `dragConfig` | `DragConfig` | — | [Drag behaviour](/react/docs/guides/dragging). |
| `resizeConfig` | `ResizeConfig` | `{ handles: ["se"] }` | [Resize behaviour](/react/docs/guides/resizing). |
| `dropConfig` | `DropConfig` | `{ enabled: false }` | [Accept external draggables](/react/docs/guides/external-drop). |
| `accept` | `(source) => boolean` | — | [dnd-kit interop](/react/docs/guides/dnd-kit-interop): also accept a foreign sortable as a drop target. |
| `compactor` | `Compactor` | `verticalCompactor` | [Packing algorithm](/react/docs/guides/compaction). |
...
| `onDragStart` `onDrag` `onDragStop` | `GridEventCallback` | — | [Drag lifecycle](/react/docs/guides/events). |
...
[Responsive layouts](/react/docs/guides/responsive).
...
`className` / `style`, or use [`useGridPlaceholder`](/react/docs/api/hooks#usegridplaceholder) for full
control.
...
shares a provider, so this is just explicit). See [Cross-grid dragging](/react/docs/guides/cross-grid).
