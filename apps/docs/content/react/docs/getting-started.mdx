...
nothing to dedupe. See [Installation](/react/docs/installation) for the full breakdown.
...
<Callout type="default">
  ...
  see [Styling](/react/docs/guides/styling) for the hooks it exposes. Want resize handles? Tiles are
  drag-only until you render them — see [Resizing](/react/docs/guides/resizing).
</Callout>

## Don't want to wire all that?

<Callout type="info">
  The turnkey [`<GridLayout>`](/react/docs/guides/component-layer) bundles the provider, the host, the
...
moves to headless). See [Migrating from RGL](/react/docs/guides/migrating-from-rgl).

## Where to next

- [Core Concepts](/react/docs/concepts) — the mental model (controlled state, the dnd-kit DNA, the `group` key) plus a gotchas cheat-sheet.
- [Headless usage](/react/docs/guides/headless) — the hooks in depth: resize handles, the placeholder, and how a dragged tile floats itself.
- [Component layer](/react/docs/guides/component-layer) — the turnkey `<GridLayout>` when you don't need custom markup.
- [Dragging](/react/docs/guides/dragging) · [Resizing](/react/docs/guides/resizing) · [Compaction](/react/docs/guides/compaction) · [Cross-grid](/react/docs/guides/cross-grid) · [dnd-kit interop](/react/docs/guides/dnd-kit-interop) · [Responsive](/react/docs/guides/responsive).
- [API Reference](/react/docs/api/overview) — every hook, component, and type.
