...
prop on the [`<GridLayout>`](/react/docs/guides/component-layer) shell):
...
- `static: true` anchors an item — others flow around it, and by default it can't be dragged or
  ...
  [Static items](/react/docs/guides/static-items).
...
`(layout, oldItem, newItem, placeholder, event, node)`. See [Events & lifecycle](/react/docs/guides/events).
