...
snapgrid is [headless-first](/react/docs/guides/headless), but you don't have to wire the hooks yourself.
...
<Callout type="info">
  **This is the escape hatch.** If the headless provider/host/tile wiring is more than you
  ...
  When one grid later needs custom markup, drop just that grid down to the [hooks](/react/docs/guides/headless).
...
[`useGridContainer`](/react/docs/guides/headless) — only the spelling differs (a JSX prop vs. an object
key). So each feature guide applies to both layers:

| Prop | What it does | Guide |
| --- | --- | --- |
| `gridConfig` | Columns, row height, margins, padding, maxRows. | [Concepts](/react/docs/concepts) |
| `dragConfig` | Handle, cancel, threshold, bounds, snap. | [Dragging](/react/docs/guides/dragging) |
| `resizeConfig` | Which handles to show. | [Resizing](/react/docs/guides/resizing) |
| `compactor` | Packing algorithm. | [Compaction](/react/docs/guides/compaction) |
| `dropConfig` / `onDrop` | Accept external draggables. | [External drop](/react/docs/guides/external-drop) |
| `isDraggable` / `isResizable` / `autoSize` | Grid-level toggles. | — |
| `onDragStart` … `onResizeStop` | Lifecycle callbacks. | [Events](/react/docs/guides/events) |

See [Components → GridLayout](/react/docs/api/components#gridlayout) for the complete prop table.

## `<ResponsiveGridLayout>`

The turnkey wrapper over [`useResponsiveLayout`](/react/docs/guides/responsive). Give it a per-breakpoint `layouts`
...
Wrap sibling grids in [`<SnapGridGroup>`](/react/docs/guides/cross-grid) — the shared provider — and tiles
...
(see [Styling](/react/docs/guides/styling)) and is memoized, so re-rendering the surface doesn't re-render
every tile.

<Callout type="info">
  There's no drag overlay to manage at either layer — a dragged tile
  [floats itself](/react/docs/guides/headless#floating). See [Components](/react/docs/api/components) for the full
  prop tables and [Headless usage](/react/docs/guides/headless) for the hooks.
</Callout>
