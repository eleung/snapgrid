...
<Callout type="info">
  Prefer the turnkey component? [`<ResponsiveGridLayout>`](/react/docs/guides/component-layer#responsivegridlayout) is
  the component that wraps this hook — pass it `layouts`, `width`, and keyed children and it handles
  the provider and the grid. See
  [Components](/react/docs/api/components#responsivegridlayout) for its full prop table.
</Callout>
