...
in-grid moves, [cross-grid](/react/docs/guides/cross-grid) hand-off, and [resizing](/react/docs/guides/resizing),
and reports results through [`onLayoutChange`](/react/docs/guides/events). That still applies here: an
...
`DragDropProvider` (or snapgrid's [`SnapGridGroup`](/react/docs/guides/cross-grid), a thin wrapper). Import
...
By default a grid only accepts grid tiles and [external-drop](/react/docs/guides/external-drop)
