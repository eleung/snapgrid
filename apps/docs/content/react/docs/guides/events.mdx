...
`useGridContainer` (and, identically, props on [`<GridLayout>`](/react/docs/guides/component-layer)).
...
```tsx
const { containerProps, group } = useGridContainer({ onLayoutChange: (next) => setLayout(next), /* … */ });
```
...
`onLayoutChange`. See [External drop](/react/docs/guides/external-drop).
