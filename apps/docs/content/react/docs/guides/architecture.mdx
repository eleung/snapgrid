...
description: How snapgrid is built on dnd-kit — the layered packages, the one-per-manager engine, the observable render bridge, and the seam a non-React binding builds on.
---
...
its own drag engine bolted on. This page is the deep dive: how the pieces fit, and the exact seam a
non-React binding (or a direct dnd-kit integration) would build on.

<Callout type="info">
  You don't need any of this to use snapgrid. `<GridLayout>` and the [headless
  hooks](/react/docs/guides/headless) are the whole API for building grids. This page is for the curious —
...
@snapgridjs/react  the binding — hooks + components that register dnd-kit entities
                   and render from the controller.  (depends on @dnd-kit/react)
```

The line that matters is the middle one. The **engine has no framework dependency**, so a Vue, Solid,
...
and other draggables under one provider ([dnd-kit interop](/react/docs/guides/dnd-kit-interop)), and what
...
That is exactly what `useGridContainer` and `useGridItem` do, wrapped in React's reactivity — a
Vue/Solid binding is the same four steps in that framework's idioms. For the full, working version,
read the [`@snapgridjs/react` source](https://github.com/eleung/snapgrid/tree/main/packages/grid-react).
