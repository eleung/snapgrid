...
[![bundle size](https://img.shields.io/bundlephobia/minzip/@snapgrid/react.svg)](https://bundlephobia.com/package/@snapgrid/react)
...
- **Pluggable packing**: `vertical` / `horizontal` / `none`, plus `masonry` / `gravity` / `shelf` from `@snapgrid/extras`, or your own `Compactor`.
...
<!-- Figures from apps/docs/components/generated/bundle-size.ts (run `pnpm --filter @snapgrid/docs measure`). The docs site reads them live; this README mirror is updated by hand. -->
...
| Package | Description |
| --- | --- |
| [`@snapgrid/react`](./packages/grid-react) | React components + hooks. The main entry point. |
| [`@snapgrid/core`](./packages/grid-core) | Framework-agnostic layout engine (geometry, move/resize, compaction). |
| [`@snapgrid/extras`](./packages/grid-extras) | Optional packers: masonry, gravity, shelf, wrap. |
