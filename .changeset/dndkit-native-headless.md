...
Rebuild the headless layer to be dnd-kit–native: grid tiles are now real dnd-kit sortables, and a dragged tile **floats itself** (dnd-kit's default feedback) rather than being mirrored by a separate overlay. The grid composes into a `DragDropProvider` you supply, instead of minting its own at the headless layer.
