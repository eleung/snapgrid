...
The framework-agnostic dnd-kit engine for [snapgrid](https://github.com/eleung/snapgrid) — the drag, resize, and cross-grid orchestration that the [`@snapgridjs/react`](https://www.npmjs.com/package/@snapgridjs/react), [`@snapgridjs/svelte`](https://www.npmjs.com/package/@snapgridjs/svelte) and [`@snapgridjs/vue`](https://www.npmjs.com/package/@snapgridjs/vue) bindings are built on, with no framework dependency of its own.
...
- **`snapMove(layout, event, ctx)`** — the consumer-facing reducer for dnd-kit ecosystem interop (drag a `useSortable` card into a grid, or a tile between a grid and a sortable list); also re-exported from `@snapgridjs/react`, `@snapgridjs/svelte` and `@snapgridjs/vue`.
- **Config & event types** — `DragConfig`, `ResizeConfig`, `DropConfig`, `GridDropData`, `GridEventCallback` (also re-exported from `@snapgridjs/react`, `@snapgridjs/svelte` and `@snapgridjs/vue`).
