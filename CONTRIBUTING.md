...
| `pnpm --filter @snapgrid/docs dev` | Run the docs site locally. |

## Project layout

```
packages/
  grid-core    @snapgrid/core    engine: geometry, move/resize, compaction, session
  grid-react   @snapgrid/react   React components + hooks
  grid-extras  @snapgrid/extras  extra packers (masonry, gravity, shelf)
apps/
...
New packing styles belong in `@snapgrid/extras`. Implement the `Compactor` interface, add a test in
